...
def train(args):
    ...
    if global_rank == 0 and args.output_dir is not None:
        ...
        swanlab.init(
            project_name="Doppelgangers++",
            experiment_name=os.path.basename(args.output_dir) if args.output_dir else "default_exp",
            config=vars(args)
        )
    else:
        log_writer = None
...
def test_one_epoch(model: torch.nn.Module, criterion: torch.nn.Module, criterion_dg: torch.nn.Module,
    ...
    if log_writer is not None:
        ...
        swanlab_test_dict = {f"{prefix}/{name}": val for name, val in results.items()}
        swanlab.log(swanlab_test_dict, step=1000 * epoch)
